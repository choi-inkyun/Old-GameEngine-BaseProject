...
//       in the dwFlags to loop the sound
//-----------------------------------------------------------------------------
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, float volume)  // daniel 2002.11.18
{
    HRESULT hr;
    BOOL    bRestored;
    ...
    LPDIRECTSOUNDBUFFER pDSB = GetBuffer( 0 );
	hr = pDSB->SetVolume( (LONG)(-2500.0f * (1.0f - volume)) );  // daniel 2002.11.18
	if(hr!=DS_OK)
	{
		switch(hr)
		{
		case DSERR_CONTROLUNAVAIL:
			::MessageBox(NULL, "DSERR_CONTROLUNAVAIL", "1", MB_OK);
			break;
		case DSERR_GENERIC:
			::MessageBox(NULL, "DSERR_GENERIC", "2", MB_OK);
			break;
		case DSERR_INVALIDPARAM:
			::MessageBox(NULL, "DSERR_INVALIDPARAM", "3", MB_OK);
			break;
		case DSERR_PRIOLEVELNEEDED:
			::MessageBox(NULL, "DSERR_PRIOLEVELNEEDED", "4", MB_OK);
			break;
		}
	}
	pDSB = GetFreeBuffer();
	...
	//hr = pDSB->SetVolume( (LONG)(-10000.0f * (1.0f - volume)) );  // daniel 2002.11.18
//    hr = pDSB->SetVolume( -8000 );  // daniel 2002.11.18
